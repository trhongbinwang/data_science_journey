...
def read_and_decode(filename):
    ...
    # reinterpret the bytes of a string as a vector of numbers -hwang
    img = tf.decode_raw(features['img_raw'], tf.uint8)
    img = tf.reshape(img, [224, 224, 3]) # reshape to image -hwang
    # img = tf.cast(img, tf.float32) * (1. / 255) - 0.5
    label = tf.cast(features['label'], tf.int32)
    return img, label

img, label = read_and_decode("train.tfrecords") # img, label just tf tensor, can use in further operation. 
